# Binary Sunset Video Wallpaper Setup
...
```text
/usr/share/binary_sunset_video.mp4
```
...
```text
binary_sunset_video.mp4
```

Verify that it exists:

```bash
ls -lh /usr/share/binary_sunset_video.mp4
```
...
```bash
mpv "/usr/share/binary_sunset_video.mp4"
```
...
```bash
mpvpaper -o "loop-file=inf --audio=yes --hwdec=auto" ALL "/usr/share/binary_sunset_video.mp4"
```
...
```ini
[Unit]
Description=MPV Binary Sunset Video Wallpaper
After=graphical-session.target

[Service]
ExecStart=/usr/local/bin/mpvpaper -o "loop-file=inf --audio=yes --hwdec=auto" ALL /usr/share/binary_sunset_video.mp4
Restart=always
RestartSec=2
