# **The Clone Wars Dynamic Wallpaper Setup**

## Pop!_OS 24.04 + mpvpaper (COSMIC/Wayland)
...
```text
/usr/share/the_clone_wars_has_begun.mp4
```
...
```text
the_clone_wars_has_begun.mp4
```

Verify that it exists:

```bash
ls -lh /usr/share/the_clone_wars_has_begun.mp4
```
...
```bash
mpv --no-audio "/usr/share/the_clone_wars_has_begun.mp4"
```
...
```bash
mpvpaper -o "loop-file=inf --no-audio --hwdec=auto" ALL "/usr/share/the_clone_wars_has_begun.mp4"
```
...
```ini
[Unit]
Description=MPV The Clone Wars Has Begun Wallpaper
After=graphical-session.target

[Service]
ExecStart=/usr/local/bin/mpvpaper -o "loop-file=inf --no-audio --hwdec=auto" ALL /usr/share/the_clone_wars_has_begun.mp4
Restart=always
RestartSec=2
...
**"The Republic saw the desperation. The Empire saw the destination."**
